...
#+begin_src scheme :tangle (if (string= system-name "Precision-3571") "./system/yggdrasil.scm" "no")

(define %efi-file-system
  (file-system
...
(define %swap-device
  (swap-space (target "/dev/nvme0n1p2")))

(define %mapped-device
  (mapped-device
    (source (uuid "86106e76-c07f-441a-a515-06559c617065"))
    (targets (list "enc"))
    (type luks-device-mapping)))

#+end_src

#+begin_src scheme :tangle (if (string= system-name "20AMS6GD00") "./system/yggdrasil.scm" "no")
...
(define %mapped-device
  (mapped-device
    (source (uuid "a9319ee9-f216-4cad-bfa5-99a24a576562"))
    (targets (list "enc"))
    (type luks-device-mapping)))
...
(operating-system
  ...
  (kernel linux)
  (kernel-arguments '("quiet" "rootfstype=btrfs"))
  ...
  (operating-system
    ...
    (skeletons
     `(;;(".config_rbw_config.json" ,rbw-config-bitwarden)
       (".config_guix_channels.scm" ,(local-file "../../.config/guix/channels.scm"))))

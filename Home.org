...
`((IMAPStore ,(string-append email-address "-remote"))
  ...
  ,(when (string? cipher-string) (CipherString cipher-string))
  ,(when (string? pipeline-depth) (PipelineDepth pipeline-depth))
  (AuthMechs LOGIN)
  (SSLType IMAPS)
...
(define (single-smtp-config email-address)
  (let* ((data_home (getenv "XDG_DATA_HOME"))
         (port
          (open-input-pipe
           (string-append "pass show" email-address " 2>/dev/null")))
...
`((account ,email-address)
  (host ,smtp-host boyer2.enpc.fr)
  (port ,smtp-port 465)
  (from ,email-address)
  (user ,email-address)
  (passwordeval ,(string-append "pass show " email-address " | head -1"))
  ,(when (string= stmp-port "465") (tls_starttls off))
  ,~#""
 )))
    ...
    (list
     (service
      ...
      (home-isync-configuration
       (config
        `(,(single-imap-config (getenv "USER_NNGRAVES"))
          ,(single-imap-config (getenv "USER_NELEVES"))
          ,(single-imap-config (getenv "USER_NGMX"))
          ,(single-imap-config (getenv "USER_NGMAIL"))
          ,(single-imap-config (getenv "USER_CPURE"))
          ,(single-imap-config (getenv "USER_QPURE"))
          ,(single-imap-config (getenv "USER_PGMAIL"))))))
     (service

      ...
      `((core
        ...
        (credential
         ((helper . ,(string-append
                     "\"!f() { test \\\"$1\\\" = get && rbw get "
                     (getenv "USER_NNGRAVES")"; }; f\""))))
        ))))))
...
(define (single-mail-config email-address)
    ...
    (list
     (service
      ...
      (home-isync-configuration
       (config
        (append
          (car (single-mail-config (getenv "USER_NNGRAVES")))
          (car (single-mail-config (getenv "USER_NELEVES")))
          (car (single-mail-config (getenv "USER_NGMX")))
          (car (single-mail-config (getenv "USER_NGMAIL")))
          (car (single-mail-config (getenv "USER_CPURE")))
          (car (single-mail-config (getenv "USER_QPURE")))
          (car (single-mail-config (getenv "USER_PGMAIL")))
          ))))
       ...
       (config
        `((defaults)
          ...
          (logfile ,(string-append (getenv "XDG_STATE_HOME")
                                   "/msmtp/msmtp.log"))
          ,@(append
             (car (cdr (single-mail-config (getenv "USER_NNGRAVES"))))
             (car (cdr (single-mail-config (getenv "USER_NELEVES"))))
             (car (cdr (single-mail-config (getenv "USER_NGMX"))))
             (car (cdr (single-mail-config (getenv "USER_NGMAIL"))))
             (car (cdr (single-mail-config (getenv "USER_CPURE"))))
             (car (cdr (single-mail-config (getenv "USER_QPURE"))))
             (car (cdr (single-mail-config (getenv "USER_PGMAIL"))))
          )))))

...
To speed up development and be sure we use the same profile without the extra-time required by =guix time-machine=, I rely on a profile stored in =.guix-profile=, which is updated through the command =make profile=.

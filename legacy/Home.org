      ...
      `((core
        ...
        (sendemail
         ((smtpserver . "ssl0.ovh.net")
          ...
          (annotate . #t)))
        (credential
         ((helper . "\"!f() { test \\\"$1\\\" = get && pass show ngraves@ngraves.fr | head -1 ; }; f\""))))
        )))))
...
#+begin_src scheme :tangle ./home/yggdrasil/ssh-utils.scm

(define-module (home yggdrasil ssh-utils)
  #:use-module (guix gexp)
  #:use-module (ice-9 popen)
  #:use-module (ice-9 rdelim)
  ...
  (list
   (service
    ...
    (home-alacritty-configuration
     (config
      `((import . #(,(local-file "../config/alacritty.yml")))))))))
           ...
           (add-to-list 'mu4e-bookmarks
                        (make-mu4e-bookmark
                         ...
                         ,(string-append "maildir:/" (list-ref %mail-list 2) "/INBOX" " OR " ;user_nngraves
                                         ...
                                         "maildir:/" (list-ref %mail-list 3) "/INBOX") ;user_ngmail
                         :key ?i))
  ...
  (list
   (simple-service
    ...
    (list
     ...
     (state-git
      (string-append (getenv "HOME") "/areas")
      "git@git.sr.ht:~ngraves/???")
     (state-git
      (string-append (getenv "HOME") "/resources/roam")
      "git@git.sr.ht:~ngraves/???")
     (state-git
      (string-append (getenv "HOME") "/archives/journal")
      "git@git.sr.ht:~ngraves/???")))))

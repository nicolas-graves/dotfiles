...
# adapted by Nicolas Graves <ngraves@ngraves.fr> for its own use
# License: GNU GPLv3
...
[ -z "$dotfilesrepo" ] && dotfilesrepo="https://github.com/nicolas-graves/voidrice.git"
[ -z "$progsfile" ] && progsfile="https://raw.githubusercontent.com/nicolas-graves/voidrice/public/.config/install/progs.csv"
...
# This allows to access to our private voidrice repo.
$aurhelper --no-confirm --needed -Syu rbw
rbw_email=$(dialog --inputbox "Please enter an email login name for the rbw user account." 10 60 3>&1 1>&2 2>&3 3>&1) || exit 1
rbw config set email "$rbw_email" && rbw login || error "Failed to get rbw user and pass."

for file in $(rbw list | grep "^id_*" ) do
	rbw get "$file" > {$HOME}/.ssh/"$file"
done;

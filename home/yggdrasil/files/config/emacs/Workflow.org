...
#+begin_src emacs-lisp :tangle no
  ...
  (defun ng/read-file-as-string (path)
    (with-temp-buffer
  ...
  (setq org-capture-templates
    `(("t" "Tasks / Projects")
      ("tt" "Task" entry (file+olp ,(ng/org-path "Projects.org") "Projects" "Inbox")
           "* TODO %?\n  %U\n  %a\n  %i" :empty-lines 1)
      ("ts" "Clocked Entry Subtask" entry (clock)
           "* TODO %?\n  %U\n  %a\n  %i" :empty-lines 1)
      ("tp" "New Project" entry (file+olp ,(ng/org-path "Projects.org") "Projects" "Inbox")
           "* PLAN %?\n  %U\n  %a\n  %i" :empty-lines 1)

      ("j" "Journal Entries")
      ("jj" "Journal" entry
           (file+olp+datetree ,(ng/get-todays-journal-file-name))
           ;"\n* %<%I:%M %p> - Journal :journal:\n\n%?\n\n"
           ,(ng/read-file-as-string "~/Notes/Templates/Daily.org")
           :clock-in :clock-resume
           :empty-lines 1)
      ("jm" "Meeting" entry
           (file+olp+datetree ,(ng/get-todays-journal-file-name))
      ...
      ("w" "Workflows")
      ("we" "Checking Email" entry (file+olp+datetree ,(ng/get-todays-journal-file-name))
...
This Org file produces a file called =ng-workflow.el= which gets loaded up in =init.el=; export it as a feature so that it can be loaded with =require=.

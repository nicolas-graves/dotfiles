...
#+PROPERTY: header-args:emacs-lisp :tangle ~/.config/emacs/ng-workflow.el
...
- =NEXT= - This task should be done next (in the Getting Things Done sense) Beware ! It's possible to have several NEXT actions depending on projects.
- =LEAD= - I'm the person responsible for setting the rythm for this task. People rely on you.
- =FLOW= - Someone is waiting for me to complete this task or for some kind of feedback on this task.
- =WAIT= - Waiting for someone else to be actionable again
- =ORGZ= - This task should in some time help to complete faster other tasks. To prioritize over unproductive tasks.
...
#+begin_src emacs-lisp
        ...
        (setq org-todo-keyword-faces
          '(("NEXT" . (:foreground "orange red" :weight bold))
            ("ORGZ" . (:foreground "HotPink2" :weight bold))
            ("LEAD" . (:foreground "orange red" :weight bold))
            ("FLOW" . (:foreground "goldenrod1" :weight bold))
            ("WAIT" . (:foreground "DarkTurquoise" :weight bold))

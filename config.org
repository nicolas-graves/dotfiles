...
*Channel commits*
Ability to pin/freeze commits with arguments of the following function.

#+begin_src scheme :noweb-ref dots-channel :noweb yes
(define* (dots-channel
          #:key
          (freeze? #f)
          (freeze-commits
...
(define (lookup var)
  (symbol->string
   (cdr
    (assoc var (cdr (assoc (gethostname) devices))))))

(define %swap-device
  (swap-space (target
               (string-append "\"" (lookup 'swap) "\""))))
...
(define file-systems
  (append
   ...
   (list (file-system
           ...
           (type "vfat")
           (device (string-append "\"" (lookup 'efi) "\""))))))
#+end_src
   ...
   (feature-custom-services
    ...
    (list
     (service
      ...
      (list
       `("guix/channels.scm" ,(plain-file "channels"

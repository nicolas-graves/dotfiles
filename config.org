...
(define my-msmtp-provider-settings
  `((ovh . ((host . "ssl0.ovh.net")
            (port . 465)
            (tls_starttls . off)))
    (gmx . ((host . "mail.gmx.net")
            (port . 587)))
    (gmail . ((host . "smtp.gmail.com")
              (port . 587)))
    (enpc . ((host . "boyer2.enpc.fr")
             (port . 465)
             (tls_starttls . off)))
    (generic . #f)))
...
(define (id->type id)
  (if (string= id "neleves") 'enpc
     (if (member id (list "ngmx" "ngmail"))
         (string->symbol (string-drop id 1))
         'ovh)))
  ...
  (define (isync-settings mail-directory mail-account)
    (let* ((id       (mail-account-id mail-account))
      ...
      `(,#~(string-append "# Account '" #$(symbol->string id)
        ...
        ,(if (string? subfolders) `(SubFolders ,subfolders) '(SubFolders Verbatim))
        ;; ,(if (list? patterns) `(Patterns ,@(string->symbol patterns) #~""))
...
(define gmx-isync-settings
  (generate-isync-serializer "imap.gmx.net" gmx-folder-mapping))

(define ovh-isync-settings
  (generate-isync-serializer "ssl0.ovh.net" gandi-folder-mapping))

(define enpc-isync-settings
  (generate-isync-serializer "messagerie.enpc.fr" gandi-folder-mapping
    #:cipher-string 'DEFAULT@SECLEVEL=1
    #:pipeline-depth 1))

(define generic-isync-settings
  (@@ (rde features mail) generic-isync-settings))

(define %isync-serializers
  `((gmail . ,gmail-isync-settings)
    (gandi . ,gandi-isync-settings)
    (gmx . ,gmx-isync-settings)
    (ovh . ,ovh-isync-settings)
    (enpc . ,enpc-isync-settings)
...
(define %main-features
  (list
   ...
   (feature-emacs-elfeed
    #:opml-feeds-file "~/.dotfiles/home/yggdrasil/files/config/elfeed.opml")
